...
import { DEPARTMENT_LIST_TOTAL } from './departmentMockData.js'

export default function DepartmentTablePagination({
  from,
  to,
  filteredTotal,
  page,
  ...
  return (
    <div className="px-6 sm:px-8 py-5 flex flex-col sm:flex-row items-stretch sm:items-center justify-between gap-4 bg-surface-container-low/30 border-t border-outline-variant/10">
      <div className="text-sm text-on-surface-variant font-medium text-center sm:text-left">
        ...
        trong <span className="font-bold text-on-surface">{filteredTotal}</span> kết quả · Tổng cấu hình{' '}
        <span className="font-bold text-on-surface">{DEPARTMENT_LIST_TOTAL}</span> tổ
      </div>
      <div className="flex items-center justify-center sm:justify-end gap-3 flex-wrap">
        <OrgTablePageSizeSelect value={pageSize} onChange={onPageSizeChange} id="dept-page-size" />
        {totalPages > 1 ? (

...
export default function StudentCreateModal({ open, onClose, onSubmit, classOptions }) {
  const titleId = useId()
  ...
  async function handleSubmit(e) {
    e.preventDefault()
    if (!student_code.trim() || !full_name.trim()) {
      setError('Nhập mã học sinh và họ tên.')
      return
    }
    if (!class_id) {
      setError('Chọn lớp.')
    ...
    try {
      ...
      setError(err?.message ?? 'Không tạo được.')
    } finally {
      setSaving(false)
    }
  }
    ...
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
      ...
      >
        ...
        <form className="mt-4 space-y-3" onSubmit={handleSubmit}>
          ...
          <div>
            <label className="block text-xs font-bold text-on-surface-variant mb-1">Mã HS *</label>
            <input className={inputClass} value={student_code} onChange={(e) => setStudentCode(e.target.value)} />
            <p className="mt-1 text-xs text-on-surface-variant">
              Cùng mã có thể thêm ở lớp khác (lên lớp / năm mới); trong một lớp chỉ một bản ghi cho mỗi mã.
            </p>
          </div>
          <div>
          ...
          <div>
            <label className="block text-xs font-bold text-on-surface-variant mb-1">Liên hệ PH (SĐT)</label>
